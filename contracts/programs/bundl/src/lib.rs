...
=======
    /// * `authorized_acquirer` - The public key of the authorized acquirer
    /// * `amount` - The amount to be deducted per interval
    pub fn initialize_controller(ctx: Context<InitializeController>, authorized_acquirer: Pubkey, amount: u64) -> Result<()> {
>>>>>>> fa724c2 (feat: initialize controller)
        // read from ctx
...
fn check_owner(authority: &Signer) -> Result<()> {
    ...
    Ok(())
=======
        if user_token_account.delegated_amount < amount {
            return Err(error!(ErrorCode::NoAllowance));
        }

        // Store config
        controller.authorized_acquirer = authorized_acquirer;
        controller.interval = 28; // 28 days
        controller.amount_per_interval = 0;
        controller.user_token_account = user_token_account.key();
        controller.bump = ctx.bumps.controller;
